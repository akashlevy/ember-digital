...
// Filename       : itrx_sync_lhl.v
// Description    : 2-flop synchronizer, 3rd flop used to create a one-clk
//                  wide pulse that detects any change in value
//                  (L->H or H->L), active LO reset to either 1's or 0's,
...
`ifndef ITRX_SYNC_LHL_V
//lint_checking USEMAC off
`define ITRX_SYNC_LHL_V
//lint_checking USEMAC on

module itrx_sync_lhl #( parameter WID       = 32'd1,        // default to one data bit
    ...
    output wire [WID-1:0]   outany  // (O) 1-clk-wide signal indicating any change in value
    );
...
generate
  for (i=0; i<WID; i=i+1) begin : i_loop
    ...
    assign outany[i] = sync_in[2] ^ sync_in[1];
